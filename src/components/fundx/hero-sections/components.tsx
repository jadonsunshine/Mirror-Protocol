...
export function ChainToggleSwitch({
  isStacksMode,
  onToggle,
}: {
  isStacksMode: boolean
...
export function HeroCTAs() {
  return (
    <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
      <Link href="/create">
        <Button size="lg" className="h-16 px-10 rounded-full text-lg shadow-[0_4px_20px_0_rgba(255,61,113,0.22)] transition-transform hover:scale-[1.025] active:scale-[0.975]">
          Start a Campaign
        </Button>
      </Link>
      <Link href="/explore">
        <Button variant="outline" size="lg" className="h-16 px-10 rounded-full text-lg border-2 border-slate-200 bg-white text-slate-700 transition-all hover:scale-[1.025] hover:border-slate-300 hover:bg-slate-50 active:scale-[0.975]">
          Explore Campaigns
        </Button>
      </Link>
    </div>
