"use client" // <--- Added this so the toggle state works
...
export function Hero() {
  const [economyToggled, setEconomyToggled] = useState(false)
        ...
        <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold tracking-tighter text-slate-900 leading-[1.1] mb-8">
          ...
          <span className="inline-flex items-center flex-wrap justify-center gap-x-4 gap-y-2">
            ...
            <span className="inline-flex align-middle ml-2">
              <div 
                onClick={() => setEconomyToggled(!economyToggled)}
                className="relative inline-flex items-center cursor-pointer group"
              >
                <div className={`w-24 h-12 rounded-full p-1 shadow-inner transition-all duration-500 hover:scale-105 ${economyToggled ? 'bg-slate-900' : 'bg-gradient-to-r from-[#FF6B4A] to-[#FF3D71]'}`}>
                  <div className={`w-10 h-10 bg-white rounded-full shadow-lg transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${economyToggled ? 'translate-x-12' : 'translate-x-0'}`} />
                </div>
              </div>
            </span>
          </span>
